...
export default function PhraseBank() {
  ...
  const generateMeta = async () => {
    const text = config.text.content
    if (!text) return
    setGeneratingMeta(true)
    setMetaError(null)
    try {
      const meta = await phrasesApi.generateMeta(text)
      setMeta({ title: meta.title, description: meta.description, tags: meta.tags })
    } catch (e: any) {
      setMetaError(e.response?.data?.error || e.message || 'Error generando metadata')
    } finally {
  ...
  return (
    <div className="flex flex-col gap-3 p-4">
      ...
      {/* Botón generar metadata */}
      <button
        onClick={generateMeta}
        disabled={generatingMeta}
        className="flex items-center justify-center gap-1.5 text-xs bg-brand-500/20 hover:bg-brand-500/30 disabled:bg-gray-700 disabled:cursor-not-allowed border border-brand-500/40 text-brand-300 rounded-lg px-3 py-2 transition-colors w-full"
      >
        <Sparkles size={13} />
